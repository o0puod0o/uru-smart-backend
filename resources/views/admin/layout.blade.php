...
<head>
    ...
    <title>@yield('title', 'ระบบจัดการ') · URU Smart</title>
    <style>
...
<body>
@auth('admin')
    <header class="topbar"><div class="wrap"><div class="brand">URU Smart Admin</div><nav class="nav"><a href="{{ route('admin.users.index') }}">จัดการผู้ใช้</a><span>{{ auth('admin')->user()->name }}</span><form method="POST" action="{{ route('admin.logout') }}">@csrf<button class="btn btn-link" type="submit">ออกจากระบบ</button></form></nav></div></header>
@endauth
<main class="@auth('admin') page @endauth"><div class="@auth('admin') wrap @endauth">
    @auth('admin')
        <nav style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">
            <a class="btn" href="{{ route('admin.dashboard') }}">แดชบอร์ด</a>
            <a class="btn" href="{{ route('admin.users.index') }}">ผู้ใช้และสิทธิ์ API</a>
            ...
            @if(auth('admin')->user()->isSuperAdmin())
                <a class="btn" href="{{ route('admin.accounts.index') }}">บัญชีผู้ดูแล</a>
            @endif
        </nav>
    @endauth
