...
class ProfileSearchController extends Controller
{
    public function index(Request $request)
    {
        ...
        $users = User::query()
            ...
            ->map(fn (User $user): array => [
                ...
                'expertises' => $user->experts->pluck('name')->values(),
    ...
    {
        match ($searchFrom) {
            'first_name' => $query->where(function ($inner) use ($keyword): void {
                $inner->where('first_name_th', 'like', "%{$keyword}%")
                    ->orWhere('first_name_en', 'like', "%{$keyword}%");
            }),
            'last_name' => $query->where(function ($inner) use ($keyword): void {
                $inner->where('last_name_th', 'like', "%{$keyword}%")
                    ->orWhere('last_name_en', 'like', "%{$keyword}%");
            }),
            'expertise_group' => $query->whereHas('experts', function ($expertQuery) use ($keyword): void {
                $expertQuery->where('has_experts.name', 'like', "%{$keyword}%");
            }),
            'interest' => $query->whereHas('interests', function ($interestQuery) use ($keyword): void {
                $interestQuery->where('name', 'like', "%{$keyword}%");
            }),
            'research' => $query->whereHas('researches', function ($researchQuery) use ($keyword): void {
                $researchQuery->where('name', 'like', "%{$keyword}%");
            }),
            'proceeding' => $query->whereHas('proceedings', function ($proceedingQuery) use ($keyword): void {
                $proceedingQuery->where('name', 'like', "%{$keyword}%");
            }),
            'journal' => $query->whereHas('journals', function ($journalQuery) use ($keyword): void {
                $journalQuery->where('name', 'like', "%{$keyword}%");
            }),
            default => $query->where(function ($inner) use ($keyword): void {
                $inner->where('first_name_th', 'like', "%{$keyword}%")
